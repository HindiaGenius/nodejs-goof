        ...
        stage('Deploy Docker Image') {
            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: "DeploymentSSHKey", keyFileVariable: 'keyfile')]) {
                    ...
                    sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no $DEPLOY_USERNAME@$TARGET_IP docker pull sryasetia/nodejsgoof:0.1'
                    ...
                    sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no $DEPLOY_USERNAME@$TARGET_IP docker run -it --detach --name nodejsgoof --network host sryasetia/nodejsgoof:0.1'
                }
            }
        }
        stage('DAST Wapiti') {
